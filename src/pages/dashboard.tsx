...
import { getDiscussionBoards } from 'apiSdk/discussion-boards';
import { DiscussionBoardInterface } from 'interfaces/discussion-board';

import { FigureInventoryListPage as ShowCaseList } from 'pages/figure-inventories';
...
function DashboardPage() {
  ...
  const discussionBoardStat = useSWR<PaginatedInterface<DiscussionBoardInterface>>(
    () => `/discussion-boards`,
    () => getDiscussionBoards({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: discussionBoardStat?.data?.totalCount,
      isLoading: discussionBoardStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Figure Inventories
                </Text>
                <NextLink href={`figure-inventories`} passHref legacyBehavior>
                  <Button
